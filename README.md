...
Overall, this step increases the robustness of the generator. Even if there are slight biases in a sample of recorded atmospheric noise, because we are feeding SHA512 more twice the amount of entropy it is outputing, the hash function is overwhelmingly likely to eliminate these biases and keep the outputs random. A cryptographic hash function like SHA-512 really helps us here because of the [avalanche effect](https://en.wikipedia.org/wiki/Avalanche_effect), in which even the smallest changes in inputs result in completely different and unrelated outputs. Therefore, if two 1536 bit chunks differ by as little as one bit, their hash outputs should be completely different.
