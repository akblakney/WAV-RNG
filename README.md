...
More specifically, the type of randomness we are after is a special kind of i.i.d. data. The two "i's" here are important. The first, *independent*, means that each value should not depend on any other value. The second, *identically*, means that all the data share the same [probability distribution](https://en.wikipedia.org/wiki/Probability_distribution). Basically, when we generate bits, we want each bit to take on a value of 1 with probability 0.5, and a value of 0 with probability of 0.5. And we don't want subsequent bits depending on each other. More on this later.
...
**Note: the testing below is done on the base .wav generator, i.e. without adding any extra options such as `--post-extract`, `--secrets`, or setting a higher `--combine` unless otherwise stated. I have done it this way because these options can only really *add* additional randomness/security, since they are more aggressive in how they extract randomness. As a sanity check, I have tested these other modes of operation during development, but it is not much use reporting those results here since they are simply stronger versions of the base generator. Suffice it to say, I have never witnessed any tests fail for any of these stronger modes of operation.**
