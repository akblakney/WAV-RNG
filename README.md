...
### -u: `use_bit`
This variable dictates which bit, within each block of .wav data, should be used. The default value is set to `None`, which means that all bits in each block will be XOR-ed together. Alternatively, the user could use `0`, as .wav data is stored in little-endian, and therefore the least significant bit will be used. The value must be a non-negative integer less than the `bits_per_block` field. The default value of `None` should be sufficient.
...
Due to the many uses of random numbers—and some of them serious consequences, as in cryptography—a great deal of effort has gone into testing candidate (pseudo)random number generators. There are different ways to test RNGs, but they typically share the same general strategy:
...
To summarize, given data that was generated by a candidate RNG, we can calculate how likely it is to observe that data, assuming that it is indeed random. If it is sufficiently unlikely—as in the case with 300 heads and 700 tails on an allegedly fair coin—we can reject the null hypothesis and conclude that the data is not random. Otherwise, we fail to reject the null hypothesis and we cannot claim the data is not random.

Notice that when we fail to reject the null hypothesis, we aren't concluding that the data is random. Instead, we simply say that we failed to prove that it's *not* random. Even when we reject the hypothesis, we do not "prove" that the data was not random. In fact, all our test statistic tells us is how likely we are to observe such data from a truly random process. It is worth remembering that if we continue to flip a truly fair coin over and over again, we will see even the most unlikely outcomes of such a process: even 300 heads and 700 tails in a string of 1,000 flips. (In fact, we can make an even stronger claim than this: the [infinite monkey theorem](https://en.wikipedia.org/wiki/Infinite_monkey_theorem) tells us that in an infinite series of fair coin flips, every possible string of outcomes would occur an infinite amount of times, with probability 1). 
...
Three popular test suites were used to test the .wav portion of the RNG. The results of all tests are available in results/. The first, ent, was not a rigorous test suite intended to deliver a pass/fail result, but is more of a sanity check on the data. The second, the NIST test suite for random number generators, failed to provide evidence of non-randomness. In particular, the success/fail rate of the tests for both the `use_bit=0` and `use_bit=None` modes were very close to the expected 99% success rate for an ideal RNG. Additionally, for both modes, the distribution of p-values did not deviate significantly from the ideal uniform distribution.

Finally, the RNG had mixed results when tested with the third and most stringent test suite, dieharder. The `use_bit=0` mode passed all tests, and had only three WEAK results out of 114. This surpassed the performance of the Python secrets module, a cryptographically secure PRNG, and was on par with other "gold standard" generators like AES_OFB (the creators of dieharder mention this generator in particular as one to compare to). The `use_bit=None` mode, however, failed two tests and had five WEAK results. The two failed tests give a strong indication that the data tested was not random, especially due to the very low p-value thresholds used for the tests. However, this does not necessarily indicate that the data generated by the .wav RNG is non-random; another possibility is that the rewinding of the file during the dieharder tests resulted in faulty test outcomes.

Verifying that a RNG is indeed random is not something one can do with 100% certainty. In the words of the creators of the dieharder tests, "perfect random number generators produce 'unlikely' sequences of random numbers—at exactly the right average rate. Testing a rng is therefore quite subtle." The best we can do is subject RNGs to statistical tests in an attempt to produce evidence that indicates that candidate generators are not perfectly random, and the lack of said evidence is not proof that the candidate generator is random. Individuals must then interpret these results and come to their own conclusions. In the future, I would like to continue testing the RNG on larger sets of data, which might produce more definitive results.
