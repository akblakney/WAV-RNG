...
In the case that the entropy of the raw bytes is less than 7 bits/byte, then the generator would probably still work decently enough to avoid detection by any randomness tests due to the SHA-512 step, but this would still be less than ideal. Therefore, users should always verify that the entropy of the raw even bytes, which can be output in the regular formats by adding the `--debug-raw` flag, is close enough to 8 bits/byte. This can be done a number of ways, with NIST's i.i.d. tests specified in 800-90b, by trying to compress with a compression tool, or with a simpler tool like `ent -c` and then finding the negative logarithm of the most likely probability. All of these methods were used earlier in this section to calculate the estimated entropy and min-entropy of the even bytes.
