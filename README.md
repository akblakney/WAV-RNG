...
Running the `rng.py` script to generate random numbers is very simple. For the most basic usage, you can run `python3 rng.py --in <input.wav>` for any input .wav file with size at least 700 bytes. This will print out, as a Python `bytearray`, all the random bytes generated by the .wav file. For a more readable format, you can choose from the `--ascii`, `--binary`, `--hex`, or `--digits` options to get the data in the specified formats. See examples below:
...
This tells us that our file `noise.wav` can generate `1242 * 64 bytes` or approximately 80kB.
...
The reason we might want to include this post-processing step is that it may be the case that the physical process we are observing—or our sampling process—may have some biases. Our samples of this physical process still have entropy, but it is difficult to guarantee that they contain i.i.d. randomness. Luckily, a hash function allows us to get samples that are basically indistinguishable from true i.i.d. randomness as long as the input entropy is high enough. To determine how much entropy we need to feed our hash function in order to guarantee a result that is sufficiently close uniform random, we can use the [leftover hash lemma](http://www.reallyreallyrandom.com/golden-rules/extract/) as reallyreallyrandom suggests. Or, we can go with NIST's rule of thumb, specified in 800-90c section 4, that we feed our hash function twice the amount of entropy we want it to output. If we assume our input source (here, the even bytes of the .wav file) have 7 bits/byte of entropy, and we feed our SHA-512 hash function 1536 bit (192 byte) blocks, this corresponds to a ratio of about 2.6 of input to output entropy in bits/byte, well over NIST's recomendation of 2:1. In fact, this is very conservative as the even bytes seem to contain close to 8 bits/byte of entropy, and we could probably get away with feeding SHA-512 128 byte blocks if we wanted to.
...
Overall, this step increases the robustness of the generator. Even if there are slight biases in a sample of recorded atmospheric noise, because we are feeding SHA512 almost twice the amount of entropy it is outputing, the hash function is overwhelmingly likely to eliminate these biases and keep the outputs random. A cryptographic hash function like SHA-512 really helps us here because of the [avalanche effect](https://en.wikipedia.org/wiki/Avalanche_effect), in which even the smallest changes in inputs result in completely different and unrelated outputs. Therefore, if two 1536 bit chunks differ by as little as one bit, theoretically their hash outputs should be completely different.
...
1. Discard the 44-byte .wav file header (in reality I discard 100 bytes to be safe).
2. Split the remaining bytes into 512-byte blocks. In each block, half of the bytes will be odd, and we won't concern ourselves with those since they don't contain much entropy. That leaves 256 bytes per block.
3. Of the 256 bytes in each block, take 192 of them and feed them to SHA-512 to get a hashed 64-byte output.
4. Now we are left with two 64-byte sets: one of them raw even bytes from the .wav file, and the other the hashed output of 192 bytes. XOR these two 64-byte blocks together. 
5. Output the product of the XOR as random.

And that's it! Overall we are hashing some of the bytes with SHA-512, and then XOR-ing the output of that hash with some of the other bytes in the .wav file. There are a few more details—like how the bytes that will be hashed and those that will not be are split up, but this is pretty minor and interested readers can look at the code for that part. The XOR step at the end might seem like an odd choice, but I chose it as a compromise. Throughout this project I've entertained many different contstructions of the RNG, each with their pros and cons. At the end I was torn between outputting raw bytes from the .wav file (and lowering the sample rate to ensure a high entropy, if needed), and having the output be entirely composed of hashes of the raw .wav bytes. Neither of these felt sufficient to me. In the first case, there is a worry that the raw bytes have a distribution that is ever so slightly different than the target uniform random. And in the second case, I didn't like the idea of the output being entirely composed of hash outputs; I wanted to retain some of the raw entropy unfiltered through a deterministic hash function. The result is, in my opinion, the best of both worlds: hash some of the bytes, and combine the result with some of the raw bytes.

We also get some nice assurances with this method. Recall that the XOR function has the nice property that it retains randomness: if we take a random string and XOR it with any other string, the result will be random. (Another way of saying this is that the resulting string will have just as much entropy as the original one.) The catch is that the two strings must be (statistically) independent. Here I claim that the output of the hashed 192 bytes is independent from the other raw 64 bytes it is XOR-ed with. There are two reasons for this. The first is that we have reason to believe that the 192 input bytes into the hash function are independent of the other 64 bytes that are not being hashed. The evidence for this is in the earlier autocorrelation plots, as well as the i.i.d. tests that were done on the data. And if the 192 input bytes are independent from the other 64 bytes, then it follows from probability theory that the hash of the 192 input bytes is also independent from the other 64 bytes.

The second reason is that, even if the 192 input bytes were *not* independent from the other 64 bytes, the 64-byte output of the hash would still be independent from the other 64 bytes. This is due to the properties of hash functions: the output of a hash should look independent of its input and from other output hashes. I say "look" because in a sense it's not: the hash is a deterministic function with transforms input bytes into output bytes. But the whole point of cryptographic hash functions is to map input strings to output strings that are indistinguishable from uniform random, such that the input cannot be derived from the output (this is related to [pre-image resistance](https://en.wikipedia.org/wiki/Cryptographic_hash_function#Properties)). For something like SHA-512, there is no actual proof that this is the case, but like with other cryptographic tools (hashes, encryption schemes, etc.), it behaves close enough to this ideal that no one can prove otherwise. 

Therefore, with the output of the hashed 192 bytes and the raw 64 bytes being independent, XOR-ing these strings together will preserve entropy. This leaves us two cases. First, suppose the raw even bytes of the file are truly i.i.d. random. In this case, XOR-ing them with the hashed output will not "downgrade" this randomness, i.e. it will not destroy any of that entropy, and the output will remain truly random. In the second case, suppose that the entropy of the raw bytes is slightly less than 8 bits/byte, and the raw bytes are not perfectly i.i.d. random. Here we assume that their entropy is at least around 7 bits/byte and therefore the output of SHA-512 retains full 8 bits/byte of entropy, per NIST's indication. Then the output of the hash function with a close to 8 bits/byte of entropy will not be "downgraded" by the raw 64 bytes it is XOR-ed with, and we will retain good RNG capabilities.

In the case that the entropy of the raw bytes is less than 7 bits/byte, then the generator would probably still work decently enough to avoid detection by any randomness tests due to the SHA-512 step, but this would still be less than ideal. Therefore, users should always verify that the entropy of the raw even bytes, which can be output in the regular formats by adding the `--debug-raw` flag, is close enough to 8 bits/byte. This can be done a number of ways, including with NIST's i.i.d. tests specified in 800-90b, by trying to compress with a compression tool, or with a simpler tool like `ent -c` and then finding the negative logarithm of the most likely probability. The simplest way is to give `rand_test.py` a file generated with the `--debug-raw` flag, which will output the min-entropy, as well as the distribution of bytes of the file. All of these methods were used earlier in this section to calculate the estimated entropy and min-entropy of the even bytes.
...
This RNG works by taking the least significant bytes of .wav file encodings of recorded atmospheric noise. These bytes on their own seem to have enough entropy, and seem to be uniformly distributed—indeed, they pass the NIST randomness test suite. But to be safe, these bytes are fed, 128 bytes at a time, to SHA-512 which serves as a post-processing/randomness extraction step to ensure the output does not contain any biases in the atmospheric waveform. The result is high quality random numbers, which have passed every available test thus far. The program `rng.py` is a wrapper around the generator that users can use to easily generate random numbers, if they have access to recorded atospheric noise. They can also use the `--secrets` option to combine the randomly generated data with pseudorandom data from Python's secrets module, a cryptographically secure PRNG. 
