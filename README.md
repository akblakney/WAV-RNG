...
With regards to entropy, I measured the entropy of the even/odd bytes by finding the min-entropy based on the empirical distribution of bytes, and the entropy-rate of the .wav data as a whole with the [paq8px compression tool](https://github.com/hxim/paq8px), which attempts to compress the data as much as possible. 
...
With the paq8px compression tool, I tried compressing the .wav files. They were able to be compressed a significant amount, down to about 33% of the original size. This gives an upper bound of 2.6 bits/byte of entropy for the overall .wav files.

To summarize, we saw the odd bytes were highly autocorrelated, and had a non-uniform byte distribution. We further saw that the odd bytes failed two i.i.d. tests, and appear to have an entropy ranging from 2 - 5.5 bits/byte. On the other hand, the even bytes were not autocorrelated and had a uniform distribution. Further, they passed two i.i.d. tests, and seem to have an min-entropy close to 8 bits/byte. However, the entropy-rate of the .wav data overall at most 2.6 bits/byte, sharply undercutting the min-entropy estimates. We do not yet have i.i.d. random data, but we have determined that the even bytes aren't autocorrelated, and we have an estimate of the entropy of the .wav data overall. We can therefore proceed to the next step where we can extract i.i.d. randomness from this entropy.
...
2. Split the remaining bytes into 1024-byte blocks.
3. Hash each 1024-byte block (with a conservative estimate of 1024 bits of entropy) to get a 64-byte digest.
4. XOR the hashed output with 64 even .wav bytes and output the result as random
5. Output the 64-byte hash output as random.
